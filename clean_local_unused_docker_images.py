...
if __name__ == "__main__":
    ...
    for i in images_all:
        ...
        for img in i["RepoTags"]:
            ...
            images_info[image_name].append({"tag":image_tag, "created":i["Created"], "Id":i["Id"], "name":img })
    ...
    #
    print flag*flag_number, "\n Containers List on %s  " % hostIp , len(containers_running_json)
    pprint.pprint([ (c["Id"], c["Image"]) for c in containers_running_json ])
    #       
    ...
    for img, tags in images_info.items():
        ...
        if img in images_used_tags:
           ...
           tags_deleted = sorted(tags, key=lambda x:x["created"], reverse=True)[2:]


        for tag in tags:

            if tag["Id"] in images_candeleted_ids:
               pass
        ...
        for tag in tags_deleted:
            ...
            if r.status_code == 200:
               ...
               print "Failed to delete %s" % tag["name"]
               print r.text
        #clean <none> images
        """
        if len(images_none):
